...
# --- Hugging Face LLM synthesis ---
def synthesize_with_hf(raw_insights):
    # Convert raw insights to a text prompt
    prompt = (
        "You are an analytics assistant. Rewrite these raw insights into concise, human-friendly summaries. "
        "Include the insight, evidence, confidence score, and recommendation.\n\n"
        f"Raw Insights:\n{json.dumps(raw_insights, indent=2)}\n\n"
        "Return each insight in a short paragraph."
    )
    ...
    try:
        synth_pipeline = pipeline(
            ...
            device=0 if os.environ.get("USE_GPU", "0") == "1" else -1  # 0=GPU, -1=CPU
        )
        result = synth_pipeline(prompt, max_new_tokens=256)
        generated_text = result[0]["generated_text"]
        # Wrap as JSON for consistency
